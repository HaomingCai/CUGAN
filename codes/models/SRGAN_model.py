...
class SRGANModel(BaseModel):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            self.fix_pre_BaseNet = self.opt['train']['fix_pre_G_BaseNet']
            self.fix_pre_CondNet = self.opt['train']['fix_pre_G_CondNet']
    ...
    def optimize_parameters(self, step):
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            pred_g_fake = self.netD(self.fake_H, cond=self.cond)
            if self.opt['train']['gan_type'] == 'gan':
                if self.gan_G_reweighting:
                    weight = 0.005 - 0.004*self.cond
                    re_weight = torch.mean(weight, dim=1)
                    l_g_gan = re_weight.view(-1, 1, 1, 1) * self.cri_gan(pred_g_fake, True)
                    l_g_gan = l_g_gan.mean()
                else:
        ...
        l_d_total = 0
        self.cond_GT = self.cond.clone()
        # self.cond_GT = self.cond_GT.fill_(0)
        pred_d_real = self.netD(self.real_H, self.cond_GT)
        pred_d_fake = self.netD(self.fake_H.detach(), self.cond)  # detach to avoid BP to G
        if self.opt['train']['gan_type'] == 'gan':
            if self.gan_D_reweighting:
                weight = 1.1*self.cond
                re_weight = torch.mean(weight, dim=1)
                l_d_real = self.cri_gan(pred_d_real, True)
                l_d_fake = re_weight.view(-1, 1, 1, 1) * self.cri_gan(pred_d_fake, False)
                l_d_fake = l_d_fake.mean()
            else:
                l_d_real = self.cri_gan(pred_d_real, True)
